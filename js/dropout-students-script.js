...
function populateTable() {
    ...
    allDropoutStudents.forEach((student, index) => {
        ...
        row.innerHTML = `
            ...
            <td>${phoneNumber}</td>
            <td><span class="badge bg-danger rounded-pill">បោះបង់ការសិក្សា</span></td>
            <td>
                <div class="action-buttons-table">
                    <button class="btn btn-success btn-sm btn-action me-1" onclick="restoreStudent('${student.key}')" 
                            title="ទាញមកចូលរៀនវិញ">
                        <i class="fi fi-rr-undo"></i>
                    </button>
                    <button class="btn btn-primary btn-sm btn-action" onclick="viewStudent('${student.key}')" 
                            title="មើលព័ត៌មាន">
                        <i class="fi fi-rr-eye"></i>
                    </button>
                    <button class="btn btn-danger btn-sm btn-premium-delete ms-1" onclick="deleteForever('${student.key}')" 
                            title="លុបជាស្ថាពរ">
...
function restoreStudent(studentKey) {
    Swal.fire({
        ...
        confirmButtonText: '<i class="fi fi-rr-check"></i> បាទ/ចាស ទាញមកវិញ',
...
function deleteForever(studentKey) {
    Swal.fire({
        title: 'លុបជាស្ថាពរ?',
        ...
        if (result.isConfirmed) {
            ...
            firebase.database().ref('students/' + studentKey).remove()
                .then(() => {
                    ...
                    Swal.fire(
                        'បានលុប!',
                        'ទិន្នន័យសិស្សត្រូវបានលុបជាស្ថាពរ។',
                        'success'
                    );
                    loadDropoutStudents();
                })
                .catch((error) => {
                    ...
                    Swal.fire(
                        'បរាជ័យ!',
                        'មានបញ្ហាក្នុងការលុបទិន្នន័យ: ' + error.message,
                        'error'
                    );
                });
        }
...
function viewStudent(studentKey) {
    ...
    const studentInfo = `
        <div class="text-start">
            ...
            <h5 class="border-bottom pb-2 mb-3 mt-4"><i class="fi fi-rr-exclamation text-danger"></i> ស្ថានភាព</h5>
    ...
    Swal.fire({
        ...
        showCancelButton: true,
        cancelButtonText: '<i class="fi fi-rr-undo"></i> ទាញមកចូលរៀនវិញ',
